...
def get_group_vars(group):
    ret_vars = {}
    group_vars = db.groups.find_one({"name": group}, {"vars": True, "_id": False})
...
if args.list:
        ...
        if group['name'] not in scanned_groups:
            ...
            for host in group_hosts:
                ...
                json_docs['_meta']['hostvars'][host] = dict(
                    list(host_vars.items() + group['vars'].items() + all_group_vars))
    ...
    host = args.host[0]
    host = db.ansible_hosts.find_one({'hostname': host}, {"vars": True, "_id": False})
    if host:
